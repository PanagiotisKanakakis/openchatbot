...
class CustomStorageAdapter(StorageAdapter):
    ...
    def filter(self, **kwargs):
        ...
        in_response_to_not_contains = kwargs.pop('in_response_to_not_contains', None)
        statement_text_not_equals = kwargs.pop('statement_text_not_equals', None)
        ...
        if in_response_to_not_contains:
            statements = statements.filter(or_(Statement.in_response_to == None,
                                               Statement.in_response_to != in_response_to_not_contains))

        if statement_text_not_equals:
            statements = statements.filter(Statement.text != statement_text_not_equals)
